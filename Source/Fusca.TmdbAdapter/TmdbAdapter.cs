...
namespace Fusca.TmdbAdapter
{
    internal class TmdbAdapter : ITmdbAdapter
    {
        ...
        public async Task<IEnumerable<FilmesGetResult>> GetFilmesAsync(FilmesGet filmesGet)
        {
            try
            {
                var cacheKey = $"filmes::{filmesGet.TermoPesquisa}::" +
                    $"{filmesGet.AnoLancamento}::{tmdbAdapterConfiguration.Idioma}";
                ...
                if (!typedCache.TryGet(cacheKey, out TmdbSearchMoviesGetResult tmdbSearchMoviesGetResult))
                {
                    ...
                    typedCache.Set(cacheKey, tmdbSearchMoviesGetResult,
                        TimeSpan.FromSeconds(tmdbAdapterConfiguration.TempoDeCacheDaPesquisaEmSegundos));
                }
            ...
            catch (ApiException e)
            {
                ...
                // Obs.: Deixar essa excecao sem tratamento, a principio nao eh errado, pois eh uma 
                // condicao nao prevista, ou seja, desconhecida.
                // Como este projeto implementa um ponto central de tratamento de erros (por meio 
                // das bibliotecas Otc.ExceptionHandler e Otc.Mvc.Filters)
                // este erro sera devidamente registrado (logs) e um identificador do registro sera 
                // fornecido na resposta. 
                // Note que em ambientes de desenvolvimento, (variavel de ambiente ASPNETCORE_ENVIRONMENT 
                // definida como Development) a excecao sera exposta na resposta, no entanto, em 
                // ambientes produtivos, apenas o identificador do log do erro sera fornecido.
                throw;
            }
        }
    }
